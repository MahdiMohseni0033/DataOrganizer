...
# 全局变量进行路径配置
label_folder = '/home/sd/Desktop/workload/rename_yolo_file/label'  # 检测结果存放文件夹labels路径

raw_images_folder = '/home/sd/Desktop/workload/rename_yolo_file/images'  # 检查图片存放文件夹raw_images路径

save_images_folder = '/home/sd/Desktop/workload/rename_yolo_file/visualized'  # 保存图片文件夹save_image路径

name_list_path = '/home/sd/Desktop/workload/rename_yolo_file/name_list_path.txt'  # 里面有检测图片名称txt文件路径

classes_path = '/home/sd/Desktop/workload/rename_yolo_file/classes.txt'
...
def draw_box_on_image(image_name, classes, colors, label_folder, raw_images_folder, save_images_folder):
    ...
    for line in source_file:  # 例遍 txt文件得每一行
        ...
        plot_one_box([x1, y1, x2, y2], image, color=colors[class_idx], label=classes[class_idx], line_thickness=1)
...
if __name__ == '__main__':  # 只有在文件作为脚本文件直接执行时才执行下面代码
    ...
    box_total = 0
    image_total = 0
    for image_name in tqdm(image_names):  # 例遍图片名称
